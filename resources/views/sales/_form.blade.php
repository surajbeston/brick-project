<h3 >Sales of {{ $dealer->name ?? "" }} ( {{ $dealer->dealer_code ?? "" }})</h3>
...
<form action="{{ route('sales.store') }}" method="post">
    ...
    <div class="form-group">
        ...
        <input type="text" class="custom-select @error('reward_rate') is-invalid @enderror" required id="reward_rate" value="{{ old('reward_rate') }}" name="reward_rate" placeholder="Reward rate">
        @error('reward_rate')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
